...
function Model ({url, position}){
  ...
  const [cameraPosition, setCameraPosition] = useState([11, 5, 11]);
  ...
  useEffect(() => {
    const handleResize = () => {
      if (window.innerWidth < 600) {
        setCameraPosition([11, 5, 11]); // Smaller screen position
        setTargetY(2)
      } else {
        setCameraPosition([1.5, 11, 1.5]); // Larger screen position
        setTargetY(3.5)
      }
    };
